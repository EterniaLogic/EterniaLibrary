...
// initialize
AbstractDBPriorityItem::AbstractDBPriorityItem(){
	priority = 0;
	data = NULL;
}
...
AbstractDBPriorityQueue::AbstractDBPriorityQueue(){
	head = NULL;
	size = 0;
}
...
void AbstractDBPriorityQueue::dataswap(AbstractDBPriorityItem *a, AbstractDBPriorityItem *b){
	// only swap data.
	void* tmpdata = a->data;
...
// remove minimum item.
void* AbstractDBPriorityQueue::pop(){
	// place last-inserted at top.
	if (size == 1){ // head item.
		void* ret = head->data;
		AbstractDBPriorityItem* last = head;
		head = 0x0;
		...
		AbstractDBPriorityItem* last = findlastinsertionposition(head);
...
void* AbstractDBPriorityQueue::top(){
	if (head == NULL) return NULL;
	return head->data;
}

// insert item into list.
void AbstractDBPriorityQueue::push(int priority, void* data){
	// pick location to add at.
	AbstractDBPriorityItem* location = head == 0x0 ? head : findinsertionposition(head);

	// create item to base off of.
	AbstractDBPriorityItem* newItem = new AbstractDBPriorityItem();
...
void AbstractDBPriorityQueue::clearQueue(){
	while (pop() != NULL);
}
...
void AbstractDBPriorityQueue::compareswap(AbstractDBPriorityItem* a, AbstractDBPriorityItem* b){
	// check the left side
	if (a->left != 0x0){
...
void AbstractDBPriorityQueue::downheap(AbstractDBPriorityItem* current){
	// repeat another time to enforce proper placement.
	compareswap(current->left, current);
	compareswap(current->right, current);
}

// sort through list to make sure that it is sorted. (Bottom up)
void AbstractDBPriorityQueue::upheap(AbstractDBPriorityItem* current){
	// go up this current node's tree section.
	if (current == 0x0) return;
	compareswap(current, current->parent);
}


// find max depth of item c from c's location.
int AbstractDBPriorityQueue::depth(AbstractDBPriorityItem* c, int initial){
...
bool AbstractDBPriorityQueue::fullAtDepth(AbstractDBPriorityItem* c){
...
AbstractDBPriorityItem* AbstractDBPriorityQueue::findinsertionposition(AbstractDBPriorityItem* current){
...
AbstractDBPriorityItem* AbstractDBPriorityQueue::findlastinsertionposition(AbstractDBPriorityItem* current){
	if (current->left == current) current->left = 0x0;
	if (current->right == current) current->right = 0x0;
...
bool AbstractDBPriorityQueue::replaceKey(AbstractDBPriorityItem* current, void* at, int key){
...
// returns the size.
int AbstractDBPriorityQueue::getSize(){
	return size;
}

// returns whether the size is zero, or the head is empty.
bool AbstractDBPriorityQueue::empty(){
