...
void MSP430::runTimer0(bool is_mclk){
    ...
    unsigned int TACTL_ = _m16(TACTL);
    unsigned int TA0CTL0_ = _m16(TA0CTL0);
    unsigned int TA0CTL1_ = _m16(TA0CTL1);
    unsigned int TA0CTL2_ = _m16(TA0CTL2);
    ...
    int clkSelect = (TACTL_ & 0x50|0x60) >> 8);
    ...
    if( (is_mclk  && (clkSelect==0)) || // TACLK == mclk
        ...
        int clkDiv = (TACTL_ & 0x30|0x40) >> 6);
        const int MCx = (TACTL_ & 0x10|0x20) >> 4);
            ...
            if(!(TA0CCTL0_ & CAP)){ // compare mode
                ...
                if(TA0R_ >= TACCR0_){
                    ...
                    switch((TA0CCTL0_ & 0x20|0x30|0x40) >> 5){
                        case 0:
                                P1OUT = TA0CCTL0_ & OUT; // direct output mode (follower)
                            break;
                        case 1:
                                P1OUT |= BIT5; // set
                            break;
                        case 2:
                                P1OUT ^= BIT5; // toggle on TACCRx / reset on TACCR0
                            break;
                        case 3:
                                P1OUT ^= BIT5; // set on TACCRx / reset on TACCR0
                            break;
                         case 4:
                                P1OUT ^= BIT5; // toggle
                            break;
                         case 5:
                            ...
                            break;
                        case 6:
                                P1OUT ^= ~BIT5; // toggle on TACCRx / set on TACCR0
                            break;
                        case 7:
                                P1OUT ^= ~BIT5; // toggle on TACCRx / set on TACCR0
                            break;
